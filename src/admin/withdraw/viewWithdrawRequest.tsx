...
export const ViewWithdrawRequest = () => {
  const [update,{data,isLoading,error}] = useUpdate('withdraw-histories')
  const router = useRouter();
  const { t } = useTranslation('common');
  const notify = useNotify();
  const refresh = useRefresh();
  
  const updateStatus = (id:number,status:DepositHistoryStatus) => {
    if(!confirm(`정말로 ${status === 'success' ? '승인':'거절'} 하시겠습니까?`)) return;
    update('withdraw-histories',{
      id,
      data: {
        status
      }
  ...
  return (
    <Stack direction={"column"}>
      <BreadCrumbs items={[{name:"회원관리"},{name:"출금요청"}]} />
      <List 
        filterDefaultValues={{
          start_date: new Date(),
          end_date: new Date()
      ...
      >
        ...
        <NumberField sx={{minWidth:120}} label={"금액"} sortable={false} source="withdraw_amount" />
        ...
        <FunctionField textAlign="right"  label="비고" render={(record:any)=>{
          return record.status === 'pending' ? 
            <Stack width={130} alignItems={"end"} spacing={0.5}>
              <ButtonGroup>
                <Button size="small" color="error" onClick={()=>updateStatus(record.id,'fail')}>거절</Button>
                <Button size="small" color="success" onClick={()=>updateStatus(record.id,'success')}>승인</Button>
              </ButtonGroup>
